...
training_data_file = open("makeyourownneuralnetwork-master/mnist_train_100.csv", 'r')
...
def emnist_train(model):
    ...
    X_train = idx2numpy.convert_from_file('gzip\emnist-digits-train-images-idx3-ubyte')
    y_train = idx2numpy.convert_from_file('gzip\emnist-digits-train-labels-idx1-ubyte')

    X_test = idx2numpy.convert_from_file('gzip\emnist-digits-test-images-idx3-ubyte')
    y_test = idx2numpy.convert_from_file('gzip\emnist-digits-test-labels-idx1-ubyte')
    ...
    #with tf.Session() as sess:
        ...
        #K.get_session().run(tf.global_variables_initializer().run())
    #model.save('emnist_letters.h5')

    print('Saved!', end='\n')
    model.fit(X_train, x_train_cat, validation_data=(X_test, y_test_cat),
              batch_size=64, epochs=3)#'''callbacks=[learning_rate_reduction],'''
...
def refact(imgName:str):

    image = Image.open(imgName)  # Открываем изображение
...
if __name__ == "__main__":
    ...
    #refact('image.jpg')
    s_out = img_to_str(model, "image.jpg")
    print(s_out)
